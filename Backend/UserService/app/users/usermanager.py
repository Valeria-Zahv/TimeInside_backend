...
class UserManager(UUIDIDMixin, BaseUserManager[models.User, uuid.UUID]):
    ...
    async def on_after_forgot_password(self, user: models.User, token: str, request=None):
        print(f"User {user.id} has forgotten their password. Reset token: {token}")

    async def on_after_request_verify(self, user: models.User, token: str, request=None):
